...
import { updateUserData } from '../../../Redux/actions';
  ...
  const EditProfile = () => {
    ...
    const [sk1, setSk1] = useState(
      state.user && state.user.skills && state.user.skills.length > 0 ? state.user.skills[0] : '>/s<',
    );
    const [sk2, setSk2] = useState(
      state.user && state.user.skills && state.user.skills.length > 1 ? state.user.skills[1] : '>/s<',
    );
    const [sk3, setSk3] = useState(
      state.user && state.user.skills && state.user.skills.length > 2 ? state.user.skills[2] : '>/s<',
    );
